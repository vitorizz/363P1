...
stockPrice_collection = mongo_db["AdjustedStockPrice"] 

try:

    cursor.execute("SELECT * FROM Adjusted_Stock_Price;") 
    Stock_Price_rows = cursor.fetchall()
    # print(Stock_Price_rows)

    cursor.execute("SELECT column_name FROM information_schema.columns WHERE table_name = 'adjusted_stock_price';") 
